(function($) {
        ...
        lazyLoad.loadUntil = function (options){
            var items = {},
                ...
                $elem.on(options.triggerEvent, loadImgFn = function(e,index,elem){
                    if(items[index] !== 'loaded'){
                        $elem.trigger(id + '-loadItems', [index, elem, function(){
                            ...
                            if (loadedItemNum === options.totalItemNum) {
                                 // 全部加载完毕
                                 floor.$doc.trigger(id + '-itemsLoaded');
                            }
                        }]);
                    }
                });  
            $elem.on(id + '-itemsLoaded',function(e){
                ...
                $elem.off(options.triggerEvent,loadImgFn);
            });
        }
        lazyLoad.isVisible = function (floorData) {
            var $win = $(window);
            /*满足window可视区高度 + 滚动条滚过的高度 > 元素上边崖距离页面顶部的距离,
            滚动条滚过的高度 < 元素上边崖距离页面顶部的距离 + 元素自身的高度，则表示元素在可视区范围内*/
        ...
        var floor = {}   
        floor.$floor = $('.floor');
        floor.$win = $(window); // 最好创建浏览器对象，这里为了方便
        floor.$doc = $(document);  
        floor.floorData = [
            {
                num: '1',
        ...
        floor.$doc.on('floors-itemsLoaded',function(e){
            floor.$win.off('scroll resize',floor.clear);
        });
        
        floor.timeToShow = function () {
            console.log('time toshou')
        ...
        floor.$win.on('scroll resize',floor.clear = function(){
            clearTimeout(floor.floorTimer);
            floor.floorTimer = setTimeout(floor.timeToShow,500);
        });
        lazyLoad.loadUntil({
            $container: floor.$doc,
    ...
    // elevator
        floor.whichFloor = function(){
            var num = -1;
            floor.$floor.each(function(index, elem) {
                var floorData = floor.floorData[index]; // 通过这个获取楼层的offsetTop
                num = index;
                // 浏览器滚动的高度 < 楼层距离页面的顶部的高度 则显示前一个楼层,也就是当前楼层索引减1
        ...
        floor.$items = floor.$elevator.find('.elevator-item');
        floor.setElevator = function(){
            ...
            if(num === -1){
                ...
                floor.$items.removeClass('elevator-active');
                floor.$items.eq(num).addClass('elevator-active'); 
            }
        }
        floor.setElevator()
        floor.$win.on('scroll resize',function(){
            clearTimeout(floor.elevatorTimer);
            floor.elevatorTimer = setTimeout(floor.setElevator,200);
        });
